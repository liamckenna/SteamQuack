...
function getDynamicFontSize(text: string) {
  ...
  const calculatedSize =
    minFontSize + (maxFontSize - minFontSize) * Math.exp(-len / decay);
...
export default function Speech() {
  const { currentLine, isActive, advance, startDialogue, isLastLine } =
    useDialogue();
  ...
  if (!isActive || !currentLine) {
    return (
      <div className="speech speech--idle">
        ...
        />
        <img
          src={speechBubbleOutline}
          alt="Speech Bubble Outline"
          className="speech__bubble-img speech__bubble-outline"
        />
        <div className="speech__content">
          <p className="speech__text" style={{ fontSize: "2rem" }}>
            ...
          </p>
        </div>
      </div>
  ...
  return (
    <div className="speech clickable" data-cy="dialogue-box" onClick={advance}>
      <div
        className="speech__bubble-img speech__bubble-bg-mask"
        style={{
          WebkitMaskImage: `url(${speechBubbleBg})`,
