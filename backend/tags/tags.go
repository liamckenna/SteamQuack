...
func CreateBaseTagWeights(gameID uint32) map[string]float64 {
	...
	for _, tag := range gameTags {
		gameTagCounts[tag.TagName] = uint32(tag.Weight)
	}
	...
	for _, gameTag := range gameTags {
		if normalizedWeight, exists := tagWeights[gameTag.TagName]; exists {
...
func GetInitialTagWeights() map[string]float64 {
	return map[string]float64{
		...
		"Action RTS":            0.0,
		"Party Game":            0.0,
		"Social Deduction":      0.0, // Kept this one in weights, missing from possible tags!
		"Well-Written":          0.0,
		"Spaceships":            0.0,
		"Reboot":                0.0,
		"Immersive":             0.0,
