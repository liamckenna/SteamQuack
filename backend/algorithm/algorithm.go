...
func CreateRecommendations(tasteProfile map[string]float64, settings map[string]any) []uint32 {
	...
	gameIDs := make([]uint32, 0, topN)
	for i := 0; i < topN; i++ {
		gameIDs = append(gameIDs, sortedScores[i].GameID)
	}
	return gameIDs
}
